...
- 🧭 **智能路由** — 自动识别题目模块（资料分析、逻辑判断、图形推理等），返回置信度和推荐路径
- 📚 **方法检索** — 292 张结构化方法卡片，覆盖行测全部核心题型
- 🏗️ **脚手架引导** — 6 大模块逐步分析，像老师一样带你拆题
- 🔒 **安全门控** — 信息不足时自动降级，绝不强行作答
...
```
用户题目 ──▶ 花生十三 MCP Server ──▶ AI 大模型（分析→作答）
                    │
               ┌────┴────┐
               │ 知识库    │
               │ 292张方法卡│
...
MCP 永远不直接输出答案，只生成引导 prompt。答案由 AI 严格遵循约束后输出。
...
│   ├── kb.py                    # 知识库加载与检索
│   ├── cli.py                   # 命令行接口
│   ├── solvers/                 # 求解器（资料分析/逻辑判断/类比/定义判断等）
